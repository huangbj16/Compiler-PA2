...
public class TypeCheck extends Tree.Visitor {
	...
	public void visitClassDef(Tree.ClassDef classDef) {
		Class parent = classDef.symbol.getParent();
		if(parent != null && parent.getSealed())
	...
	public void visitGuardedStmt(Tree.GuardedStmt guardedStmt) {
		if(guardedStmt.branch.size() != 0) {
			for (Tree ifsubstmt : guardedStmt.branch) {
	...
	public void visitIfSubStmt(Tree.IfSubStmt ifSubStmt) {
		...
		if(!ifSubStmt.expr.type.equal(BaseType.BOOL))
			issueError(new BadTestExpr(ifSubStmt.expr.getLocation()));
	}
...
//		System.out.println(assign.left.type);
		assign.left.accept(this);
		assign.expr.accept(this);
		if(assign.left.type == BaseType.UNKNOWN){
			Symbol name = table.lookup(((Tree.Var)assign.left).name, false);
			name.setType(assign.expr.type);
...
//		if(!var.type.equal(BaseType.UNKNOWN))//which means not defined in symbol tables, so it must be used in a wrong place instead of lvalue;
...
//		System.out.println(scopy.loc.toString());
		Symbol symbol = table.lookup(scopy.ident, true);
		if(symbol != null){
			if(symbol.getType().isClassType()){
				scopy.expr.accept(this);
				if(symbol.getType().equal(scopy.expr.type)){
					scopy.type = symbol.getType();
				}
				else {
					issueError(new BadScopySrcError(scopy.loc, symbol.getType().toString(), scopy.expr.type.toString()));
				}
			}
			else {
				issueError(new BadScopyArgError(scopy.loc, "dst", symbol.getType().toString()));
			...
			issueError(new UndeclVarError(scopy.loc, scopy.ident));
			scopy.type = BaseType.ERROR;
		}
	}
...
//		System.out.println("binary"+left.type);
//		System.out.println("binary"+right.type);
