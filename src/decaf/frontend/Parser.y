...
%token IN
%token FOREACH
%token COMPPARENLEFT COMPPARENRIGHT
%token '+'  '-'  '*'  '/'  '%'  '='  '>'  '<'  '.'
%token ','  ';'  '!'  '('  ')'  '['  ']'  '{'  '}' ':'
...
ForeachStmt     :   FOREACH '(' BoundVariable IN Expr WHILE Expr ')' Stmt
                    {
                        $$.stmt = new Tree.ForeachStmt($3.ident, $3.type, $5.expr, $7.expr, $9.stmt, $1.loc);
                    }
                |   FOREACH '(' BoundVariable IN Expr ')' Stmt
                    {
                        $$.stmt = new Tree.ForeachStmt($3.ident, $3.type, $5.expr, null, $7.stmt, $1.loc);
                    }
                ;
...
GuardedStmt     :   IF '{' IfBranchList IfSubStmt '}'
                    {
                        $$.blist = $3.blist;
                        $$.blist.add($4.stmt);
                        $$.stmt = new GuardedStmt($$.blist, $1.loc);
                    }
                |   IF '{' '}'
                    {
                        $$ = new SemValue();
                        $$.stmt = new GuardedStmt(new ArrayList<Tree>(), $1.loc);
                    }
                ;
...
IfBranchList    :   IfBranchList IfBranch
                ...
                |   /* empty */
                    {
                        $$ = new SemValue();
                        $$.blist = new ArrayList<Tree>();
                    }
                ;

IfBranch        :   IfSubStmt ORT
...
Expr            :	LValue
                ...
                |   Expr TWOMOD Expr
                    {
                        $$.expr = new Tree.ArrayRepeat($1.expr, $3.expr, $3.loc);
                    }
                |   Expr TWOPLUS Expr
                    {
                        $$.expr = new Tree.ArrayConcat($1.expr, $3.expr, $3.loc);
                    }
                |   Expr '[' Expr ':' Expr ']'
                    {
                ...
                |   '[' Expr FOR IDENTIFIER IN Expr ']'
                    {
                        $$.expr = new Tree.ArrayComp($2.expr, $4.ident, $6.expr, $1.loc);
                    }
                ;
